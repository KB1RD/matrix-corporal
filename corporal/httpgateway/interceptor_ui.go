...
func (me *UiAuthInterceptor) Intercept(r *http.Request) InterceptorResponse {
	...
	// The JSON has to be accessed like this since we don't know every field
	auth, auth_ok := payload["auth"].(map[string]interface{})
	if !auth_ok {
		// Not having an auth field is valid
	...
	if auth["type"].(string) != "m.login.password" {
		// If the user is not trying to do a password login, Corporal doesn't do anything
		return InterceptorResponse{
			Result:               InterceptorResultProxy,
			LoggingContextFields: loggingContextFields,
		}
	...
	// Now, all fields should be present for password login
	pass, pass_ok := auth["password"].(string)
	ident, ident_ok := auth["identifier"].(map[string]interface{})
	if !ident_ok || !pass_ok {
		return createInterceptorErrorResponse(loggingContextFields, matrix.ErrorBadJson, "Bad input")
	}
